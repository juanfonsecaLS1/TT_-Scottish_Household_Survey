...
```{r}
TL_summary_mode = data |> filter(mainmode<7) |> 
  summarise(mean_dist = weighted.mean(roadnet_kms,trav_wt,na.rm = T),
            .by = c(dyear,mainmode)) 

TL_summary_mode |> 
   mutate(mainmode = haven::as_factor(mainmode)) |> 
  ggplot(aes(x = dyear,y=mean_dist,col= mainmode))+
  geom_line()+
...
## Trip length using stage data

```{r}
TL_summary_stages <- data.stage |> 
  filter(mode < 7) |> 
  summarise(avg.dist = weighted.mean(stage_distance,trav_wt,na.rm = T),
            .by = c(mode,purpose_old,dyear))
```


```{r}
TL_summary_stages |>
   mutate(mode = haven::as_factor(mode),
          purpose_old = haven::as_factor(purpose_old)) |> 
  ggplot(aes(x=dyear,y = avg.dist,col = mode))+
  geom_line()+
  facet_wrap(purpose_old~.)+
  theme(legend.position = "bottom")
```
